...
extern __device__ const uint64_t keccak_f_round_constants[24];
...
public:
    ...
    template<unsigned int rate>
    __device__ void shake(uint8_t *out, size_t out_len, const uint8_t *in, size_t in_len, uint8_t *tmp_shared) {
...
template<unsigned int rate, uint8_t p>
__device__ __forceinline__ void shake(uint8_t *out, size_t out_blocks, const uint8_t *in, size_t in_len) {
    const int8_t x = threadIdx.x % 5;
    const int8_t y = threadIdx.x / 5;
    const int8_t theta1 = (x + 1) % 5;// (x + 1) mod 5
    const int8_t theta2 = (x + 2) % 5;// (x + 2) mod 5
