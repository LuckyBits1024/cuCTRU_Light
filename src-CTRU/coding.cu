...
__device__ static uint32_t abs_q2(uint32_t x)
{
    uint32_t mask = -(((CTRU_Q2 - (x << 1)) >> 31) & 1);
    return (mask & (CTRU_Q2 - x)) | ((~mask) & x);
}

__device__ static uint32_t const_abs(int32_t x)
{
    uint32_t mask = x >> 31;
    return (x ^ mask) - mask;
}

__device__ static uint8_t decode_d8_00(uint32_t *cost, uint32_t tmp_cost[8][2])
{
...
__device__ static uint8_t decode_d8_10(uint32_t *cost, uint32_t tmp_cost[8][2])
{
    uint8_t m[2] = {0, 0}, xor_sum = 0, res;
    uint32_t min_diff = ~0U >> 2, r;
    uint32_t min_i = 0;
